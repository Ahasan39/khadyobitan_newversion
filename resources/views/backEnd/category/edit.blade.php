                            ...
                            <div class="col-sm-12">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="summernote form-control @error('meta_description')  is-invalid @enderror" name="meta_description"
                                        rows="6" id="meta_description">{!! $edit_data->meta_description !!}</textarea>
        ...
        <select class="form-control select2-tags" name="tags[]" multiple="multiple">
            @foreach($allTags as $tag)
                <option value="{{ $tag->name }}" 
                    {{ in_array($tag->id, $edit_data->tags->pluck('id')->toArray()) ? 'selected' : '' }}>
                    {{ $tag->name }}
                </option>
            @endforeach
        </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" value="1" name="front_view"
                                            @if ($edit_data->front_view == 1) checked @endif>
                                        <span class="slider round"></span>
                                    </label>
                                    @error('front_view')
                                        <span class="invalid-feedback" role="alert">
