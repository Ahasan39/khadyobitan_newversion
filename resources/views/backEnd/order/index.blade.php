...
@section('content')
                            ...
                            <div class="col-sm-8">
                                <ul class="action2-btn">
                                    <li><a data-bs-toggle="modal" data-bs-target="#asignUser"
                                            class="btn rounded-pill btn-success"><i class="fe-user"></i> Assign</a>
                                    </li>
                                    <li><a data-bs-toggle="modal" data-bs-target="#changeStatus"
                                            class="btn rounded-pill btn-primary"><i class="fe-folder-plus"></i> Change
                                            Status</a>
                                    </li>
                                    <li><a href="{{ route('admin.order.bulk_destroy') }}"
                                            class="btn rounded-pill btn-danger order_delete"><i class="fe-trash"></i> Delete
                                        </a></li>
                                    ...
                                    @endif
                                    <form style="display:inline-block" action="{{ route('admin.courier.status_update') }}"
                                        method="POST"> @csrf
                                        <button class="btn rounded-pill btn-success"
                                            onclick="return confirm('Are you sure want courier status auto update?')">
                                            <i class="fe-refresh-cw"></i> Courier Update
                                        </button>
                                    </form>
   ...
   title="Edit"><i class="fe-edit"></i></a>
                                  ...
                                  <td>
                                    @php
                                        $phone = $value->shipping ? $value->shipping->phone : '';
                                        
                                        $phoneOrders = $phone ? \App\Models\Shipping::where('phone', $phone)
                                                            ->orderBy('created_at', 'ASC') 
                                                            ->pluck('id') : collect();
                                        
                                        $currentOrderIndex = $phoneOrders->search($value->id) + 1; // 1-based index
                                    @endphp
                                    
                                    <p class="repeat__customer">
                                        {{ $phone }}
                                        @if($phoneOrders->count() > 0)
                                            <span class="badge bg-info" title="Order sequence: {{ $currentOrderIndex }} of {{ $phoneOrders->count() }}">
                                                {{ $currentOrderIndex }}
                                            </span>
                                                <i class="fe-check ms-1" title="Customer Order Sequence"></i>
                                        @endif
                                    </p>
                                    
                                    <button class="fraud_checker btn btn-xs rounded-pill btn-primary"
                                        data-id="{{ $value->id }}" data-bs-toggle="modal"
                                        data-bs-target="#fraud_checker_modal">Check Fraud</button>
                                </td>
    ...
    <form>
        <div class="modal fade" id="fraud_checker_modal" tabindex="-1" aria-labelledby="fraud_checker_modal"
            aria-hidden="true">
    ...
    <script>
        $(".fraud_checker").on("click", function() {
            var id = $(this).data('id');
