                                        ...
                                        <div class="details_slider owl-carousel">
                                            @foreach ($details->variableimages as $value)
                                                <div class="dimage_item">
                                                    <img src="{{ asset($value->image) }}" class="block__pic" loading="lazy" decoding="async" />
                                                </div>
                                            @endforeach
                                        </div>
                                        <div
                                        ...
                                        <div class="product-cart">
                                            ...
                                            <form action="{{ route('cart.store') }}" method="POST" name="formName">
                                              ...
                                              @php
                                                ...
                                                $companyName = $whatsAppSetting->name ?? 'Webleez Top';
                                                $defaultMessage = $whatsAppSetting->message ?? 'Hello, I am interested in your product';
                                                $productUrl = url()->current();
                                                $message = urlencode("{$defaultMessage} from {$companyName}. Here is the link: {$productUrl}");
                                            @endphp
                                            
                                            <div class="mt-md-2 mt-2">
                                                <h4 class="font-weight-bold d-flex gap-2">
                                                    <!-- Call Button -->
                                                    <a class="btn btn-success w-50" href="tel:{{ $contact->hotline }}">
                                                        <i class="fa-solid fa-phone"></i> Call
                                                    </a>
                                            
                                                    <!-- WhatsApp Button -->
                                                    <a class="btn btn-success w-50" 
                                                       href="https://wa.me/88{{ preg_replace('/\D/', '', $contact->hotline) }}?text={{ $message }}" 
                                                       target="_blank">
...
@push('script')
    ...
    <script type="text/javascript">
     ...
     <script type="text/javascript">
// Add to Cart Data Layer Event
$(document).ready(function() {
    $('.add_cart_btn').on('click', function() {
        // Get product details dynamically
        var productName = "{{ $details->name }}";
        var productId = "{{ $details->id }}";
        var productPrice = "{{ $details->variable_count > 0 && $details->type == 0 ? $details->variable->new_price : $details->new_price }}";
        var quantity = $('input[name="qty"]').val();
        var currency = "BDT";
        
        // Calculate total value
        var totalValue = parseFloat(productPrice) * parseInt(quantity);
   ...
   <script>
    toastr.options = {
        ...
        "showEasing": "swing",
        "hideEasing": "linear",
        "showMethod": "fadeIn",  // smooth effect
        "hideMethod": "fadeOut"
    };
    ...
    function sendSuccess() {
        if (document.forms["formName"]["product_size"]) {
            let size = document.forms["formName"]["product_size"].value;
            if (size === "") {
        ...
        if (document.forms["formName"]["product_color"]) {
            let color = document.forms["formName"]["product_color"].value;
            if (color === "") {
...
<!-- Example for ViewContent on a product page -->
<script>
