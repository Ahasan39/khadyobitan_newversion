...
const Index = ({
  ...
  const mapProduct = (p: any) => ({
    ...
    category: p.category?.name || p.category || (typeof p.category_id === 'number' ? 'Category ' + p.category_id : "General"),
    weight: p.weight || "1kg",
    ...
    images: p.images ? p.images.map(extractImagePath) : (p.image ? [extractImagePath(p.image)] : ['/placeholder.svg']),
      ...
      <div>
          ...
          <div className="absolute bottom-3 sm:bottom-5 right-4 sm:right-8 flex gap-2 z-10">
            {heroSlides.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-500 ${i === currentSlide ? "bg-white w-6 sm:w-8" : "bg-white/40 hover:bg-white/60"
                  }`}
        ...
        <section className="py-8 sm:py-12 overflow-hidden">
          ...
          <div className="relative w-full">
            <div 
              className="flex gap-6 animate-marquee-categories"
