...
use App\Http\Controllers\Admin\CheckoutContentController;
...
use App\Http\Controllers\Admin\ThemeColorSettingController;
...
use App\Http\Controllers\UpdateLicenseController;
use App\Http\Controllers\UpdatePermissionController;
use App\Http\Controllers\Admin\CustomerReviewController;
use App\Http\Controllers\Admin\NoticeController;
use App\Http\Controllers\Admin\OfferController;
use App\Http\Controllers\Frontend\CheckoutLeadController;
use App\Http\Controllers\Frontend\ProductShareController;
...
// Other Auth Routes (Register, Password Reset, etc.)
...
Route::get('/create-storage-link', function () {
    try {
        ...
        return "Storage:link command executed successfully.";
    } catch (\Exception $e) {
...
// ==================== CUSTOMER API ROUTES (kept for functionality) ====================
Route::group(['prefix' => 'customer', 'namespace' => 'Frontend', 'middleware' => ['ipcheck', 'check_refer']], function () {
    // Redirect old URLs to new Inertia routes
    Route::get('/login', fn() => redirect('/login'))->name('customer.login.redirect');
    Route::get('/register', fn() => redirect('/register'))->name('customer.register.redirect');
    Route::get('/checkout', fn() => redirect('/checkout'))->name('customer.checkout.redirect');
    Route::get('/order-track', fn() => redirect('/track-order'))->name('customer.order_track.redirect');
    
    // API/functional routes (these don't render views)
    ...
    Route::post('/logout', [CustomerController::class, 'logout'])->name('customer.logout.old');
    ...
    Route::post('/order-save', [CustomerController::class, 'order_save'])->middleware('web')->name('customer.ordersave');
...
Route::group(['prefix' => 'customer', 'namespace' => 'Frontend', 'middleware' => ['customer', 'ipcheck', 'check_refer']], function () {
    // Redirect old URLs to new Inertia routes  
    Route::get('/account', fn() => redirect('/account'))->name('customer.account.redirect');
    Route::get('/orders', fn() => redirect('/account/orders'))->name('customer.orders.redirect');
    ...
    Route::get('/profile-edit', fn() => redirect('/account/profile-edit'))->name('customer.profile_edit.redirect');
    Route::post('/profile-update', [CustomerController::class, 'profile_update'])->name('customer.profile_update');
    Route::get('/change-password', fn() => redirect('/account/change-password'))->name('customer.change_pass.redirect');
...
Route::group(['namespace' => 'Admin', 'middleware' => ['auth', 'lock', 'check_refer'], 'prefix' => 'admin'], function () {
    ...
    Route::post('order-auto-update', [OrderController::class, 'status_update'])->name('admin.courier.status_update');
    ...
    Route::post('incomplete-orders/{id}/add-note', [\App\Http\Controllers\Admin\IncompleteOrderController::class, 'addNote'])->name('admin.incomplete_orders.add_note');
    Route::post('incomplete-orders/{id}/mark-contacted', [\App\Http\Controllers\Admin\IncompleteOrderController::class, 'markAsContacted'])->name('admin.incomplete_orders.mark_contacted');
    Route::post('incomplete-orders/destroy/{id}', [\App\Http\Controllers\Admin\IncompleteOrderController::class, 'destroy'])->name('admin.incomplete_orders.destroy');
    ...
    Route::get('customer-review/edit/{id}', [CustomerReviewController::class, 'edit'])->name('customer_reviews.edit');
    ...
    Route::get('feature-toggles/{id}/edit', [FeatureToggleController::class, 'edit'])->name('feature.toggles.edit');
    Route::post('feature-toggles/{id}/update', [FeatureToggleController::class, 'update'])->name('feature.toggles.update');
